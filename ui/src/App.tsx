...
import { Bug, CheckSquare2, Goal, Moon, Plus, Sparkles, Sun } from "lucide-react"
...
type Draft = Pick<Item, "title" | "type" | "description" | "parent_id" | "project_id"> & { id?: string }
...
function App() {
  ...
  const counts = useMemo(() => Object.fromEntries((Object.keys(typeMeta) as ItemType[]).map((type) => [type, visibleItems.filter((item) => item.type === type).length])), [visibleItems])
  const blockedParents = useMemo(() => {
    if (!draft?.id) return new Set<string>()
    const blocked = new Set([draft.id])
    const addChildren = (id: string) => visibleItems.filter((item) => item.parent_id === id).forEach((item) => { blocked.add(item.id); addChildren(item.id) })
    addChildren(draft.id)
    return blocked
  }, [draft?.id, visibleItems])
  ...
  useEffect(() => {
    ...
    localStorage.setItem("work-items-theme", dark ? "dark" : "light")
  }, [dark])

  const openItem = (item: Item) => { setError(""); setExpanded(false); setDraft({ ...item }) }
  const openNew = (parentId: string | null = null) => {
    if (!projectId) return setProjectDialog(true)
    setError(""); setExpanded(false); setDraft({ title: "", type: "Task", description: "", parent_id: parentId, project_id: projectId })
  }
  const save = async () => {
    if (!draft) return
    setError("")
    try {
      const item = await request<Item>(draft.id ? `/api/items/${draft.id}` : "/api/items", { method: draft.id ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(draft) })
      await load()
      setDraft(item)
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not save item") }
  }
  const createProject = async () => {
    setError("")
    try {
      const project = await request<Project>("/api/projects", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: projectName }) })
      setProjectName(""); setProjectDialog(false); await load(); setProjectId(project.id)
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not create project") }
  }

  return <div className="min-h-svh bg-[#f8f8f8] text-foreground"><div className="min-h-svh bg-background"><main className="min-w-0"><header className="flex min-h-14 flex-wrap items-center gap-3 border-b px-4 py-2 sm:px-6"><div className="min-w-32"><h1 className="text-sm font-semibold">Work items</h1><p className="text-[11px] text-muted-foreground">Local-first workspace</p></div><Select value={projectId} onValueChange={(id) => { setProjectId(id); setDraft(null) }} disabled={!projects.length}><SelectTrigger aria-label="Project" className="w-48"><SelectValue placeholder="Select a project" /></SelectTrigger><SelectContent>{projects.map((project) => <SelectItem key={project.id} value={project.id}>{project.name}</SelectItem>)}</SelectContent></Select><Button variant="outline" size="sm" onClick={() => { setError(""); setProjectDialog(true) }}><Plus />Project</Button><div className="ml-auto flex items-center gap-2"><Sun className="size-4 text-muted-foreground" aria-hidden="true" /><Switch checked={dark} onCheckedChange={setDark} aria-label="Dark theme" /><Moon className="size-4 text-muted-foreground" aria-hidden="true" /><Button onClick={() => openNew()} className="gap-1.5"><Plus />New item</Button></div></header><div className="mx-auto max-w-5xl px-4 py-7 sm:px-8"><div className="flex flex-col gap-5 border-b pb-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm text-muted-foreground">{currentProject ? `${currentProject.name} work` : "Create a project to begin."}</p><h2 className="mt-1 text-2xl font-semibold tracking-tight">{currentProject?.name || "Projects"}</h2></div>{currentProject && <div className="flex gap-2 overflow-x-auto pb-1">{(Object.keys(typeMeta) as ItemType[]).map((type) => <Badge key={type} variant="outline" className="shrink-0 gap-1.5 py-1"><TypeMark type={type} size="small" />{counts[type]} {type}{counts[type] === 1 ? "" : "s"}</Badge>)}</div>}</div><section className="mt-5" aria-label="Work item tree">{loading ? <p className="py-10 text-sm text-muted-foreground">Loading your workspace…</p> : !currentProject ? <div className="rounded-xl border border-dashed px-6 py-14 text-center"><span className="mx-auto mb-3 grid size-10 place-items-center rounded-xl bg-muted"><Goal className="size-5 text-muted-foreground" /></span><h3 className="font-semibold">Create your first project</h3><p className="mt-1 text-sm text-muted-foreground">Projects keep independent work trees separate.</p><Button className="mt-4" onClick={() => setProjectDialog(true)}><Plus />New project</Button></div> : visibleItems.length ? <Tree items={visibleItems} parentId={null} depth={0} onSelect={openItem} onAddChild={openNew} /> : <div className="rounded-xl border border-dashed px-6 py-14 text-center"><span className="mx-auto mb-3 grid size-10 place-items-center rounded-xl bg-muted"><Goal className="size-5 text-muted-foreground" /></span><h3 className="font-semibold">Begin with a work item</h3><p className="mt-1 text-sm text-muted-foreground">{currentProject.name} is ready for its first Epic, Feature, Task, or Bug.</p><Button className="mt-4" onClick={() => openNew()}><Plus />Create item</Button></div>}</section></div></main></div>
