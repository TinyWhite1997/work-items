...
import { Bug, CheckSquare2, Columns3, Goal, LayoutList, Moon, Plus, Sparkles, Sun } from "lucide-react"
...
function App() {
  ...
  const load = async () => {
    setLoading(true)
    try { const [nextProjects, nextItems] = await Promise.all([request<Project[]>("/api/projects"), request<Item[]>("/api/items")]); setProjects(nextProjects); setItems(nextItems); setProjectId((current) => nextProjects.some((project) => project.id === current) ? current : (nextProjects[0]?.id || "")) }
  ...
  const openNew = (parentId: string | null = null) => { if (!projectId) return setProjectDialog(true); setError(""); setExpanded(false); setDraft({ title: "", type: "Task", description: "", parent_id: parentId, project_id: projectId, status: "open", priority: "medium" }) }
  const save = async () => { if (!draft) return; setError(""); try { const item = await request<Item>(draft.id ? `/api/items/${draft.id}` : "/api/items", { method: draft.id ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(draft) }); await load(); setDraft(item) } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not save item") } }
